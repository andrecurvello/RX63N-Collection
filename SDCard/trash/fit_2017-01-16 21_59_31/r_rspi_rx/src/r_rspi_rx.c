...
*
* Copyright (C) 2013(2014-2016) Renesas Electronics Corporation. All rights reserved.
***********************************************************************************************************************/
/***********************************************************************************************************************
* File Name    : r_rspi_rx.c
* Device(s)    : RX Family
* Tool-Chain   : Renesas RX Standard Toolchain 2.05.00
* OS           : None
* H/W Platform :
...
#ifdef BSP_MCU_RX64M
void rspi_spei_64m_isr(void *pdata);
#endif
#ifdef BSP_MCU_RX71M
void rspi_spei_71m_isr(void *pdata);
#endif
#ifdef BSP_MCU_RX65N
void rspi_spei_65n_isr(void *pdata);
#endif
/***********************************************************************************************************************
...
#if defined BSP_MCU_RX65N
            new_reg_settings->spdcr2_val = p_setregs_struct->spdcr2_val;
#endif
        }
        break;
...
static void rspi_interrupts_clear(uint8_t channel)
{
    switch (channel)
    {
        ...
        case 0:
            ...
            #endif
            #if defined BSP_MCU_RX64M || defined BSP_MCU_RX71M || defined BSP_MCU_RX65N
                /* Clear the error interrupt status flag. */
                ICU.GRPAL0.BIT.IS17 = 0;
            #endif
    ...
    switch (channel)
    {
        ...
        case 0:
            ...
            if(enabled)
            {
                ...
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI0_SPEI0, (bsp_int_cb_t)rspi_spei_64m_isr);
                #elif defined BSP_MCU_RX71M
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI0_SPEI0, (bsp_int_cb_t)rspi_spei_71m_isr);
                #elif defined BSP_MCU_RX65N
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI0_SPEI0, (bsp_int_cb_t)rspi_spei_65n_isr);
                #else
                #endif
        ...
        case 1:
            ...
            if(enabled)
            {
                /* Register the error callback function with the BSP group interrupt handler. */
                #if defined BSP_MCU_RX64M
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI1_SPEI1, (bsp_int_cb_t)rspi_spei_64m_isr);
                #elif defined BSP_MCU_RX71M
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI1_SPEI1, (bsp_int_cb_t)rspi_spei_71m_isr);
                #elif defined BSP_MCU_RX65N
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI1_SPEI1, (bsp_int_cb_t)rspi_spei_65n_isr);
                #else
                #endif
                /* Enable error interrupt source bit */
        ...
        case 2:
            ...
            if(enabled)
            {
                /* Register the error callback function with the BSP group interrupt handler. */
                #if defined BSP_MCU_RX65N
                R_BSP_InterruptWrite(BSP_INT_SRC_AL0_RSPI2_SPEI2, (bsp_int_cb_t)rspi_spei_65n_isr);
                #else
                #endif
                /* Enable error interrupt source bit */
...
******************************************************************************/
    ...
    void rspi_spei_64m_isr(void *pdata)
    {
        if (IS(RSPI0, SPEI0))
        {
            rspi_spei_isr_common(0);    // RX64M only has channel 0.
        }
    } /* end rspi_spei_64m_isr */
#endif
#ifdef BSP_MCU_RX71M
    /* Called from BSP group interrupt handler. */
    void rspi_spei_71m_isr(void *pdata)
